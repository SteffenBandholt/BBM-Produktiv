## 2026-09-08 – SiGeKo S2.2: Projektformular technisch umgesetzt, manuelle Abnahme offen
...
Native Editorfensterbedienung nicht nachgewiesen. Manuelle S2.2-Abnahme aus dem
Detailplan noch offen: npm run test:sigeko:s2.2:form führt durch das isolierte
Altprojekt, getrennte Beginndaten und Wiederöffnen. PR bleibt bis dahin Draft;
kein Merge, kein S2.3. Rechnung #275, PDF, Mail, Protokollsettings und Vorlagen
...
- Der Versuch, die 186-mm-Tabelle auf 187 mm zu verbreitern, blieb ohne Zustandsaenderung und zeigte die konkrete Seitenrandmeldung. Der normale Protokollabschluss erzeugte danach eine echte zweiseitige Produkt-PDF unter `C:\Downloads\bbm\M86-DIAG - M86 Diagnose\Protokolle`; Sichtpruefung und PDF-Metadaten bestaetigen A4 und lueckenlose gespeicherte Teilnehmertracks. Commit und Push: keiner.

## PDF-Nutzfläche und lückenlose Teilnehmer-Tabelle

- Status: `[A]` im Adapter und realen Produkt-Print-DOM nachgewiesen; A4 bleibt 210 × 297 mm. Die tatsächlich wirksamen Ränder oben/rechts/unten/links = 5/12/0/12 mm definieren X = 12…198 mm, Y = 5…297 mm und 186 × 292 mm Nutzfläche.
- Der gemeinsame PDF-Geometriepfad weist horizontale und vertikale Randverletzungen vor jeder Zustandsübernahme atomar mit einer konkreten Randmeldung ab. Auch Änderungen der vorhandenen PDF-Randeinstellungen werden gegen alle registrierten Ziele geprüft. Es gibt kein stilles Clamping und keinen teilweise übernommenen Zustand.
- Die bestehende Teilnehmer-Tabelle ist als Tabelle mit fünf expliziten Tracks registriert: Name 34, Funktion 32, Firma 30, Telefon/E-Mail 72 und Anwesend/Verteiler 18 mm. Innere Grenzen ändern direkte Nachbarn gegenläufig bei fester Gesamtsumme; die äußere Breite ändert Tabelle und rechten Track gemeinsam. Header, Datenzellen und Hintergründe bleiben lückenlos und innerhalb der rechten Nutzflächenkante 198 mm.
- Reale Print-DOM-Messung: Baseline x ≈ 11,998 mm, Breite ≈ 186,003 mm, rechte Kante ≈ 198,001 mm. Die Grenze Telefon/E-Mail | Anwesend/Verteiler wurde mit +1 und −1 mm geprüft; die Tabellenaußenkanten blieben unverändert. 185 mm Tabellenbreite ergab 17 mm Außenspalte und rechte Kante ≈ 197 mm. Die rechte Überschrift ließ sich separat um 1 mm verschieben, ohne die Tabelle zu verändern.
- Grün: M81 14/14, kombinierte M81/M81.1/M82.4 47/47, M85 einschließlich aller 47 unveränderten Golden-Fixtures und neuer Teilnehmer-Bounding-Box-Nachweise, UI-Editor-Vertragscheck. Der bekannte vertikale TOP-Readback-/Overlaypunkt bleibt getrennt unverändert. Commit und Push: keiner.

## PDF-Tabellenspalten als vollständige Bedieneinheit

- Status: `[A]` im realen isolierten BBM-Protokoll über zwei vollständige Anwendungsstarts abgenommen; bestehende `TableColumn`-/`Table`-/`Label`-Struktur weiterverwendet.
- Die horizontale Meta-Innengeometrie verwendet im Body die vollständige verfügbare Zellbreite; der frühere `11ch`-Deckel ist entfernt, Datum und Verantwortlicher stehen auf derselben Inhaltsbreite und der Statuspunkt bleibt über einen festen kompakten Statustrack mit konstantem Abstand dem Status zugeordnet. Der getrennte vertikale Readback-Punkt `Registry-y = 91 mm` gegenüber `DOM-y ≈ 79 mm` ist dokumentiert, aber in diesem Paket nicht verändert.
- Horizontale Spaltenbearbeitung ist keine freie Translation mehr. Die Auswahl einer `TableColumn` bietet ausschließlich die verständliche Operation `Spaltengrenze verschieben`; `resizeColumnBoundary` ändert die beiden direkten Nachbarbreiten atomar und gegenläufig. Tabellenbreite und Außenkanten bleiben unverändert, Header- und Datenzellen verwenden dieselben lückenlosen Tracks. Sichtbarkeit wirkt weiterhin auf die vollständige Spalte; Überschriften bleiben explizite Kindziele für Textposition, Schriftgröße, Ausrichtung und Sichtbarkeit.
- Der native Inspector zeigt für `Status / Fertig bis / verantw` reale Werte statt `– mm`: Position `157,08 / 91 mm`, Breite `40,92 mm`, Höhe `120 mm`, Sichtbarkeit, `TOP-Tabelle` und aktuelle Spaltenbreite. Freie horizontale Pfeile und der direkte Breitenmodus sind bei der Spaltenauswahl ausgeblendet; sichtbar sind `← Grenze nach links` und `→ Grenze nach rechts` mit der bestehenden Millimeter-Schrittweite.
- Reale Bedienabnahme im isolierten Produktprotokoll: Baseline `Gegenstand 120,9 mm / Meta 40,92 mm`; Grenze `+1 mm` ergibt `121,9 / 39,92 mm`, Undo stellt `120,9 / 40,92 mm` her, Grenze `−1 mm` ergibt `119,9 / 41,92 mm`. In allen Zuständen bleiben Spaltensumme und Tabellenbreite 186 mm; Kopf, Zellen und echte zweiseitige BBM-PDF sind lückenlos. Save und separater zweiter Programmstart stellen `119,9 / 41,92 mm` und die Grenze bei `144,08 mm` exakt wieder her.
- Grün: M81, M85 einschließlich aller 47 Golden-Fixtures und neuer ±1-mm-Print-DOM-Nachweise sowie native K17-/M87-Regressionen. Der vollständige `npm test` bleibt an bereits vorhandenen, paketfremden Erwartungsabweichungen unter anderem bei Registry-Anzahl/Fingerprints, Lizenzdokument-Hash und Harness-/Public-API-Anzahl rot; die dafür verantwortlichen Bereiche wurden in diesem Paket nicht geändert. Der bekannte zu hohe Auswahlrahmen beziehungsweise `Registry-y = 91 mm` gegenüber realem DOM-y bleibt als separater vertikaler Readback-/Overlaypunkt offen und wurde nicht mit dieser horizontalen Reparatur vermischt. Commit und Push: keiner.

## Produktiver Druckpfad und gespeicherter PDF-Pool

- Status: `[A]` im realen isolierten Electron-BBM praktisch geprüft; die fachliche Nutzerfreigabe bleibt offen. Der laufende Protokollstand bleibt eine temporäre Vorschau. `Protokoll beenden` erzeugt weiterhin automatisch Protokoll, Firmenliste, ToDo-Liste und TOP-Liste.
- Die drei Listen erhalten wie das Protokoll einen eindeutigen Abschlussstand aus Projektnummer, Kurzname, PDF-Art, Protokollnummer und Datum. Zwei reale Abschlüsse des neu über die BBM-Oberfläche angelegten Projekts `QA-160826 - Druckpfad` ergaben acht getrennte Dateien für Stand #1 und #2 im vorhandenen Projektbaum `bbm/<Projekt>/Protokolle|Listen`.
- `Drucken` verwendet wieder den vorhandenen gespeicherten Projektbestand: zuerst PDF-Art, dann konkrete Fassung, anschließend direkte Vorschau genau dieser Datei. Hauptmenü und Protokoll-Quicklane laufen über denselben Weg. Reale Auswahl und Vorschau bestanden für die ältere Firmenliste #1, Protokoll #1, ToDo-Liste #1 und TOP-Liste #1; nach dem Schließen blieb kein veraltetes Auswahl-Popup zurück.
- Das bestehende Editorprofil bleibt Zusatz zum Drucksystem. Ein gültiges gespeichertes Profil wird auf den normalen Protokoll-DOM angewandt; fehlt es oder ist es unlesbar/inkompatibel, läuft der normale Produktdruck mit seinem Baseline-Layout weiter. Es wurde weder eine zweite Layoutquelle noch eine neue Druckarchitektur eingeführt.
- Reale Editor-/Produktdruck-Abnahme über einen vollständigen Neustart: `Dokumenttitel` auf `x=21 mm` und `16,5 pt` geändert, `Seite · Wert` auf `visible=false` gesetzt, gespeichert und Editor geschlossen. Die danach über den normalen Protokollabschluss erzeugte echte Projekt-PDF zeigt den vergrößerten/verschobenen Titel und nur `Seite` ohne Seitenwert.
- Grün: Gruppe `layout-output`, M85-Satzvertrag mit 47 Golden-Fixtures, UI-Editor-Vertragscheck, `git diff --check` und gezieltes ESLint mit 0 Fehlern. `npm test` bleibt ausschließlich an bereits paketfremd veralteten UI-Editor-Sollzahlen/Fingerprints, Licensing-Hashes, Navigationserwartungen und Harness-Zählungen rot. Commit und Push: keiner. Nächster Schritt: fachliche Sichtfreigabe; keine weitere Druck-/Pfadarchitektur ohne eigenen Auftrag.
...
### M86.11 - Protokoll-TOP-Zeile sichtbar wie Restarbeiten

- Status: `[A]`; die produktive Protokoll-TOP-Zeile besitzt genau drei sichtbare logische Hauptspalten nach dem bestehenden Restarbeiten-Prinzip. Links stehen Nummer, vorhandenes Anlagedatum, Klasse, Markierung und Ebene-1-Schalter vertikal; mittig Kurztext ueber Langtext; rechts Termin, Status mit vorhandener Kennzeichnung und Verantwortlich vertikal. Es wurden weder Restarbeiten-Code noch Restarbeiten-Werte kopiert oder geaendert.
- Tabelle, Tabellenkopf, Datenbereich, Zeilenvorlage, drei Spalten und alle sichtbaren Einzelziele sind komponentennah vollstaendig deklariert und an echte DOM-Refs gebunden. IDs und Parents sind stabil, wiederholte Zeilenziele sind echte Multi-Refs, Spalten besitzen gemeinsame explizite Breitenquellen. Erlaubt sind ausschliesslich vorhandene Layoutoperationen; Fachaktionen, Reihenfolge, Topologie und Scrollbesitz bleiben gesperrt.
- Auswahlrahmen verwenden nur positive sichtbare Zielgeometrie. Ausgeblendete oder ausserhalb des Scrollclips liegende bedingte Ziele erzeugen keinen Root-/Ersatzrahmen. Gespeicherte Operationen fuer ausgeblendete bedingte Multi-Refs werden beim Start logisch gehalten und beim naechsten sichtbaren Rerender auf die realen Knoten angewandt; interaktive Bearbeitung bleibt bis dahin blockiert.
- Die deklarierten Schrift-Baselines entsprechen den wirklichen CSS-DIP-Werten. Sichtbar zeigte der Langtext `12,667 DIP`; Plus, Minus, Verschieben und Undo funktionierten. Save stellte `13,667 DIP` im automatischen zweiten Start mit `startup_layout_applied` wieder her. Eine Aenderung auf `14,667 DIP` wurde ohne Speichern verworfen; der neu geoeffnete Editor las wieder `13,667 DIP`.
- Sichtpruefung: Restarbeiten-Referenz und Protokoll bei 1920x1080, 1600x900, 1366x768 und 900x430; alle Zeilen-, Spalten- und Einzelziele elementgenau ausgewaehlt. Ein-/Ausklappen, ToDo-/Alle-Filter und Multi-Ref-Rerender blieben stabil. Beide isolierten Acceptance-Starts endeten mit Exitcode 0 und das Temp-Profil wurde entfernt.
- Neuer Guardrail `scripts/tests/m86-11ProtokollRestarbeitenRowGeometry.test.cjs`: 11/11 Prueffelder. Die Gruppe `ui-editor-m51-m80` sowie `npm test` und `npm run test:node` sind gruen; beide Gesamtsuiten 8/8. Keine PDF-, Fachlogik- oder Restarbeiten-Aenderung. `docs/licensing.md` und `design-reference/` blieben unangetastet. Commit/Push/PR/Merge: keiner.

### M86.11b - Produktive Dreispaltenstruktur ausdruecklich beibehalten

- Status: `[A]`. Die im Arbeitsbaum bereits vorhandene, sichtbar erfolgreiche M86.11-Tabellen-/Zeilenstruktur bleibt ausdruecklich produktiver Protokollbestandteil. Ein separater technischer M86.11a-Rueckbau war weder im Branch noch im Diff vorhanden und musste deshalb nicht zurueckgenommen werden; insbesondere wurden keine M86.11-Wrapper, Parents, Breitenquellen oder direkten Refs entfernt oder neu erfunden.
- Der sichtbare Tabellenkopf benennt die drei Produktzonen jetzt eindeutig mit `Nr. / Datum / Klasse`, `Gegenstand` und `Fertig bis / Status / Verantw.`. Normale TOP-Zeilen behalten links Nummer, Datum und Klasse, mittig Kurztext ueber Langtext sowie rechts Fertig bis, Status mit Kennzeichnung und Verantwortlich. Der vorhandene Listenbereich bleibt alleiniger vertikaler Scrollbesitzer; eine responsive Ersatzstapelung und horizontaler Tabellen-Scroll sind ausgeschlossen.
- Der neue Guardrail `scripts/tests/m86-11bProtokollProductiveTableStructure.test.cjs` prueft den produktiven Tabellenkopf, echte DOM-Rechtecke aller drei Zeilenspalten, die vertikale Reihenfolge der Inhalte, direkte sichtbare Refs ohne Root-/Tabellenersatz sowie die produktive Strukturquelle und das unveraenderte Restarbeiten-Referenzprinzip.
- Sichtabnahme vor und mit Editor: Restarbeiten-Referenz und Protokoll im Vollbild, Protokoll zusaetzlich bei 1600x900, 1366x768 sowie 900x430 DIP. Im Kleinformat blieb die normale, innerhalb des bestehenden Listenbereichs erscrollte TOP-Zeile dreispaltig. Plus/Minus, Verschieben, Undo, Original, Save, automatischer zweiter Start und Verwerfen wurden am Langtext sichtbar geprueft; gespeicherte `13,667 DIP` wurden im zweiten Start und nach dem Verwerfen von `14,667 DIP` wieder gelesen. Beide abschliessenden Acceptance-Starts endeten mit Exitcode 0 und entfernten ihr isoliertes Profil.
- Tests: M86.11b 5/5, M86.11 11/11, M86.10 15/15, M86.9 7/7, M86.8 4/4 und M83 16/16; Restarbeiten-Modul und `ui-editor-m51-m80` gruen; `npm test` 8/8 und `npm run test:node` 8/8; Vertrags-Selbsttest gruen; gezieltes ESLint ohne Fehler (10 bereits sichtbare Warnungen). Keine PDF-, Fachlogik-, Restarbeiten- oder Registry-Topologieaenderung. `docs/licensing.md` wurde weder bearbeitet noch gestaged; `design-reference/` blieb unberuehrt. Commit/Push/PR/Merge: keiner.

### M86.12 - Dreispaltenraster im normalen Source-Start repariert

- Status: `[A]`. Der vom Nutzer bereits mit `npm start` nachgewiesene Blockfluss wurde im selben normalen lokalen Profil reproduziert. Alte EXEs, Installer und Packaging waren nicht Ursache dieses Source-Befunds und wurden nicht untersucht.
- Runtime-Ursache: Das gespeicherte Tabellenlayout lieferte den gueltigen vollstaendigen Text-Track `minmax(0, 1fr)`. `applyProtokollTopsUiLayout()` setzte ihn korrekt am gemeinsamen Tabellencontainer; `tops.css` verschachtelte ihn jedoch erneut als `minmax(0, var(...))`. Der daraus entstehende Wert `minmax(0, minmax(0, 1fr))` machte die gesamte explizite Trackliste ungueltig. Chromium erzeugte deshalb fuer Header und TOP-Zeile nur je einen impliziten Vollbreitentrack. Es gab keine spaetere CSS-Regel, Inline-Ueberschreibung, doppelte Stylesheetladung oder falsche Klasse.
- Reparaturregel: Persistierte Spaltenwerte werden vor dem produktiven Setzen als vollstaendige CSS-Tracks validiert; positive numerische Werte erhalten `px`, ungueltige oder unvollstaendige Altwerte fallen auf die drei Vertragsdefaults zurueck. Ein persistierter kompletter Alt-Descriptor wird nicht uebernommen. Der validierte Text-Track wird im Header und Zeilengrid direkt eingesetzt und nicht erneut in `minmax()` eingeschlossen. Gueltige gespeicherte Einzeltracks bleiben wirksam.
- Reale Source-Abnahme: Vorher lagen Header- und normale TOP-Zellbereiche jeweils an derselben X-Position; nachher berechnete Chromium im normalen `npm start` fuer den Header `64,996 / 301,004 / 96 px` und fuer die normale TOP-Zeile `64,996 / 299,560 / 96 px`. Links, Mitte und rechts besitzen getrennte X-Positionen und erfuellen beide Ueberlagerungsgrenzen. Nummer/Datum/Klasse stehen links, Kurztext ueber Langtext mittig sowie Fertig bis/Status/Ampel/Verantwortlich rechts.
- Editorabnahme: Die drei Tabellenspalten sowie TOP-Nummer, Anlagedatum, Klasse, Kurztext, Langtext, Fertig bis, Status, Ampel und Verantwortlich wurden im echten Editorbaum einzeln ausgewaehlt. Eine reversible Aenderung der linken Spalte um `+10 DIP` liess drei Tracks bestehen; Undo stellte die Ausgangsbreite wieder her. Es wurde nicht gespeichert.
- Der neue Guardrail `scripts/tests/m86-12ProtokollSourceGridRuntime.test.cjs` verwendet Chromium fuer die echte CSS-Berechnung und reproduzierte den Fehler vor der Reparatur. Er prueft normales gespeichertes Layout, drei X-Positionen, Nichtueberlagerung, Ablehnung ungueltiger Alttracks und Wirksamkeit gueltiger gespeicherter Tracks. M86.12 3/3, M86.11b 5/5, M86.11 11/11, M86.10 15/15, M86.9 7/7 und M86.8 4/4 sind gruen; `npm test` und `npm run test:node` jeweils 8/8; Vertrags-Selbsttest gruen; gezieltes ESLint ohne Fehler (drei bestehende `eqeqeq`-Warnungen). Keine PDF-, Fachlogik-, Restarbeiten- oder Registry-Topologieaenderung. Commit/Push/PR/Merge: keiner.
...
## Rechnung - korrigierte effektive Button-Entgrenzung nach Nutzerwiderlegung

- Status: umgesetzt und im normalen produktiven RechnungScreen erneut real abgenommen. Der frühere Abschluss zu `6a9dfd0f` war falsch, weil der damalige Harness CoreShell-/Popup-Styles nicht lud und nur `< 20`/`< 18` statt `6 x 6 px` forderte. Im Produktpfad erzwang der Popupstandard weiterhin 30 px Mindesthöhe.
- Reparatur: Globale CoreShell- und Popup-Mindesthöhen gelten nicht mehr für `.invoice-button`; die Rechnung behält ohne Editoroperation ihre 30-px-Standardhöhe. Explizit editierte Gridbuttons verwenden `justify-self`/`align-self`, Flexbuttons den exakten Flex-Basis-/Align-Self-Weg. Text, Padding, Rahmen, feste Dezimaltracks und Intrinsic-Sizing setzen die sichtbare Außenbox nicht mehr zurück.
- Realnachweis: Alle 16 Buttons übernehmen im Chromium-Guard `0 x 0 px`, messen bei Soll `6 x 6` jeweils `5.996 x 5.996 px` und übernehmen auch große Sollwerte. Der normale Router/RechnungScreen mit nativem WPF-Editor prüfte Nachkommastellen, `+Titel`, `+Position`, Schieben, Löschen, Proberechnung und Kopfbutton; sichtbares Speichern, Rechnung-Reopen und ein zweiter Electron-Prozess erhielten die etwa 6-px-Werte. Das Nicht-Button-Referenzziel `rechnung.editor.headerCanvas` folgt derselben Semantik.
