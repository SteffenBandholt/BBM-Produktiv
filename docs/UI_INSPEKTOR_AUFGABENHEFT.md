...
- K19.96 abgeschlossen: Der sichtbare G75-Stand ist in `docs/UI_EDITOR_PDF_PLAN_PAGE_1_READONLY_SICHTPRUEFUNG.md` dokumentiert und testseitig abgesichert; die Surface-Auswahl zeigt `Restarbeiten - PDF Plan Seite 1`, waehrend die SurfaceInfo weiterhin `restarbeiten.ui.main` bleibt.
- K19.97 abgeschlossen: Das SurfaceInfo-Verhalten nach der read-only Freigabe ist in `docs/UI_EDITOR_SURFACE_INFO_VERHALTEN_ENTSCHEIDUNG.md` als offene Entscheidungsgrundlage dokumentiert; empfohlen bleibt, die SurfaceInfo vorerst nicht umzubauen.
- K19.98 abgeschlossen: Das bestehende UI-Editor-Panel zeigt zusaetzlich einen kleinen read-only Hinweis fuer `pdf.plan.page.1`; SurfaceInfo bleibt bewusst `restarbeiten.ui.main`, und `docs/UI_EDITOR_PDF_PLAN_PAGE_1_READONLY_HINWEIS_REFERENZSTAND.md` dokumentiert Sichtstand, Guardrails und Electron-Sichtpruefung.
- K19.99 abgeschlossen: Die getrennte manuelle Zielrouten-Sichtpruefung ist in `docs/UI_EDITOR_PDF_PLAN_PAGE_1_MANUELLE_SICHTPRUEFUNG.md` dokumentiert; BBM, Launcher und Projekte-Ansicht waren sichtbar, die konkrete Restarbeiten-Zielroute war in dieser Sitzung aber nicht reproduzierbar erreichbar und ist deshalb ehrlich als `nicht vollstaendig bestanden / nicht vollstaendig erreichbar` festgehalten.
- K19.100 abgeschlossen: Die konkrete Restarbeiten-Zielroute ist in `docs/UI_EDITOR_RESTARBEITEN_ZIELROUTE_SICHTPRUEFUNG.md` reproduzierbar dokumentiert; der Pfad `Start` -> `Projekte` -> Projektkachel `Nr.: 04-2026 / UI-Polish fuer BBM` -> `Restarbeiten` -> `UI-Editor` fuehrt zum read-only Zielstand `Restarbeiten - PDF Plan Seite 1` mit `restarbeiten.ui.main`.
- K19.101 abgeschlossen: Die read-only Abnahmereferenz ist in `docs/UI_EDITOR_PDF_PLAN_PAGE_1_READONLY_ABNAHME_REFERENZSTAND.md` gebuendelt; `pdf.plan.page.1` bleibt nur read-only sichtbar, `restarbeiten.ui.main` bleibt Host-/Bestandssurface, `plan.canvas.default` bleibt blockiert und die manuelle Sichtpruefung aus G80 wird referenziell abgeschlossen.
- K19.102 abgeschlossen: `plan.canvas.default` wird in `docs/UI_EDITOR_SURFACE_FREIGABE_KANDIDAT_PLAN_CANVAS_DEFAULT.md` nur als naechster Kandidat bewertet; es gibt keine Policy-Aenderung, keine neue Freigabe und keine neue Sichtbarkeit.
- K19.103 abgeschlossen: `plan.canvas.default` ist jetzt explizit read-only sichtbar freigegeben; `docs/UI_EDITOR_PLAN_CANVAS_DEFAULT_READONLY_POLICY_REFERENZSTAND.md` dokumentiert die neue Policy, waehrend `restarbeiten.ui.main` Host-/Bestandssurface bleibt.
- K19.104 abgeschlossen: Der Zwei-Surface-read-only-Stand ist in `docs/UI_EDITOR_PLAN_CANVAS_DEFAULT_READONLY_SICHTPRUEFUNG.md` dokumentiert; `pdf.plan.page.1` und `plan.canvas.default` bleiben read-only sichtbar, `restarbeiten.ui.main` bleibt Host-/Bestandssurface.
- K19.105 abgeschlossen: Die gesamte read-only Surface-Phase ist in `docs/UI_EDITOR_SURFACE_READONLY_PHASE_ABNAHME_REFERENZSTAND.md` als Abschluss-/Referenzstand gebuendelt; `restarbeiten.ui.main` bleibt Host-/Bestandssurface, `pdf.plan.page.1` und `plan.canvas.default` bleiben read-only sichtbar.
...
- K19.115 abgeschlossen: G121 dokumentiert die spaetere Host-Kontext-UEbergabe als Freigabeentscheidung; `docs/UI_EDITOR_HINWEIS_INFOTEXT_HOST_UEBERGABE_FREIGABEENTSCHEIDUNG.md` begrenzt Quelle, Bedingungen, Pruefungen und blockierte Speicherwege, ohne Produktivlogik, sichtbare UI oder echte Host-Uebergabe.
- K19.116 abgeschlossen: G122 schliesst die echte Host-Kontext-UEbergabe aus dem Restarbeiten-Host an den UI-Editor an; `docs/UI_EDITOR_HINWEIS_INFOTEXT_HOST_KONTEXT_UEBERGABE_RESTARBEITEN_REFERENZSTAND.md` dokumentiert den technischen Uebergabepunkt, den sichtbaren Kontext und die weiter blockierten Speicherwege.
...
## Statusupdate K19.84
- `docs/UI_EDITOR_SURFACE_SWITCH_LAUNCHER_REFERENZSTAND.md` dokumentiert den G64-Stand als stabilen read-only Launcher-Referenzstand.
- Der Launcher nutzt `buildReadonlySurfaceSwitchResultForLauncher(...)` nur intern read-only als vorgeschaltete Referenz vor der SurfaceSelection.
...
- Erlaubt bleibt ausschliesslich `restarbeiten.ui.main`.
- `pdf.plan.page.1`, `plan.canvas.default`, unbekannte SurfaceIds, `*` und leere IDs bleiben blockiert und werden auf `restarbeiten.ui.main` zurueckgefuehrt.
- `docs/UI_EDITOR_SURFACE_SWITCH_READONLY.md` dokumentiert Datenfluss, Ergebnisformen, Sicherheitsgrenzen und Nicht-Ziele.
...
### K19.113 - Grundlagen 1/3: Editor-Bauplan und Ziel-App-Anbindung erstellen
- Status: erledigt
- Beschreibung:
  ...
  - Der Stand bleibt read-only: `restarbeiten.ui.main` ist Host-/Bestandssurface,
    `pdf.plan.page.1` und `plan.canvas.default` sind nur Kontextanzeigen.
  - Keine Produktivlogik, keine sichtbare UI, keine echte Surface-Umschaltung,
    kein Drag, kein Resize, keine Persistenz und keine DB-/IPC-Schreibwege.
- Betroffene Dateien:
