...
| `PDF-V2-PROT-003` | TOP-Reihenfolge entspricht den in `printData` normalisierten/sortierten Druckdaten. Der per `tops.special_type = audio_import` und aktueller Elternkette erkannte Import-Unterbaum wird vor Vorschau und Protokollsatz entfernt; unter einen normalen Titel verschobene Punkte sind reguläre Druckdaten. Level 1 nutzt eine eigene unteilbare Zeile mit Keep-with-next; Unterpunkte besitzen Nummer, Kurz-/Langtext und Meta. | `printData`, `_buildTopRowData`, PrintShell, Audioimport-Filtertests | fest |
