...
export class EmployeeDialog {
  constructor({ documentRef, title = "Mitarbeiter bearbeiten", onSubmit, onCancel } = {}) {
  ...
  _build(title) {
    ...
    this.root = mkEl(doc, "section", "employee-dialog");
    this.root.setAttribute("data-component", "employee-dialog");

    this.titleEl = mkEl(doc, "h3", "employee-dialog-title", title);
    this.form = new EmployeeForm({ documentRef: doc });

    this.actions = mkEl(doc, "div", "employee-dialog-actions");
    this.saveBtn = mkEl(doc, "button", "employee-dialog-btn employee-dialog-btn-save", "Speichern");
    this.cancelBtn = mkEl(doc, "button", "employee-dialog-btn employee-dialog-btn-cancel", "Abbrechen");
  ...
  setCompanyOptions(companies) {
    this.form.setCompanyOptions(companies);
  }
