...
import { inspectM80ScopeRegistration, createM80RegistrationDescriptor } from "../../src/renderer/ui-editor/m80HostAdapter.js";
import { resetM80PilotWorkingStatesForDiagnostic } from "../../src/renderer/ui-editor/m80Refs.js";

const outputPath = process.env.BBM_PROTOKOLL_REGISTRY_DIAGNOSTIC_RESULT;

function sampleTops() {
  return [
    { id: "diag-title", level: 1, displayNumber: 1, title: "Diagnose Titel", longtext: "Titeltext", created_at: "2026-08-09" },
    { id: "diag-top", level: 2, displayNumber: "1.1", title: "Diagnose TOP", longtext: "Diagnose Langtext", created_at: "2026-08-09", due_date: "2026-08-12", status: "offen", responsible_label: "Diagnose", is_task: 0, is_decision: 0 },
  ];
}
...
async function main() {
  ...
  screen.store.setState({
    ...
    meetingMeta: { meeting_number: 1, meeting_date: "2026-08-09", keyword: "Diagnose", context_label: "Diagnose" },
  });
  ...
  const resolvedScope = registration.registryScopes.find((entry) => entry.scopeId === "protokoll.edit.root") || null;
  ...
  const result = {
    ...
    shortCounter: scope.elements.find((entry) => entry.id === "protokoll.edit.short.counter") || null,
    longCounter: scope.elements.find((entry) => entry.id === "protokoll.edit.long.counter") || null,
  };
