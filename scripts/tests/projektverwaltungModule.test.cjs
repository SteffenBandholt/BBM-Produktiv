...
async function runProjektverwaltungModuleTests(run) {
  ...
  await run("Projektverwaltung: MainHeader rendert Version, Aktiv-Kontext und Kundentext", async () => {
    ...
    try {
      ...
      assert.equal(header.elRightInfo.style.top, "8px");
      assert.equal(header.root.style.gridTemplateRows, "auto auto auto");
      assert.equal(header.root.style.rowGap, "3px");
      assert.equal(header.root.style.padding, "8px 12px 4px");
      assert.equal(header.elVersion.style.fontSize, "12px");
      assert.equal(header.elVersion.style.lineHeight, "14px");
      assert.equal(header.elVersion.style.fontWeight, "500");
      ...
      assert.equal(header.elActive.style.lineHeight, "14px");
