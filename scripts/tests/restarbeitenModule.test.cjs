...
async function runRestarbeitenModuleTests(run) {
  ...
  await run("M28 Filterleiste: links Klassenfilter, Mitte Verortung, rechts Metafilter (nicht globaler Header)", async () => {
    ...
    assert.match(styleSource, /restarbeiten-listHeader__panel--meta\{outline-color:rgba\(249,115,22,.42\);background:rgba\(255,243,224,.82\);margin-left:0;width:max-content;justify-self:end;padding:2px 4px 2px;align-self:end;grid-column:3 \/ span 2;grid-row:2\}/);
    assert.match(styleSource, /restarbeiten-filterleiste__field\{display:grid;width:100%;min-width:0;gap:1px/);
    ...
    assert.match(styleSource, /restarbeiten-filterleiste__metaFilters > \.restarbeiten-filterleiste__field:nth-child\(1\)\{background:#eef6ff;box-shadow:inset 0 0 0 1px rgba\(59,130,246,.18\)\}/);
    assert.match(styleSource, /restarbeiten-filterleiste__metaFilters > \.restarbeiten-filterleiste__field:nth-child\(2\)\{background:#fff4e8;box-shadow:inset 0 0 0 1px rgba\(249,115,22,.18\)\}/);
    assert.match(styleSource, /restarbeiten-filterleiste__metaFilters > \.restarbeiten-filterleiste__field:nth-child\(3\)\{background:#eff9ef;box-shadow:inset 0 0 0 1px rgba\(34,197,94,.18\)\}/);
    assert.match(styleSource, /restarbeiten-filterleiste__metaFilters > \.restarbeiten-filterleiste__field:nth-child\(4\)\{background:#f4efff;box-shadow:inset 0 0 0 1px rgba\(168,85,247,.18\)\}/);
    ...
    assert.match(styleSource, /restarbeiten-filterleiste__metaFilters\{display:grid;width:max-content;box-sizing:border-box;padding-left:0;grid-template-columns:80px\s+70px\s+110px\s+96px;gap:4px;justify-content:end;min-width:0;align-items:end;background:rgba\(255,252,245,.72\);outline:1px dashed rgba\(245,158,11,.34\);outline-offset:-1px\}/);
