...
async function runEditorLabV2AccessTests(run) {
  ...
  await run("EditorLab V2: MainHeader enthaelt keinen DEV-Headerbutton mehr", () => {
    assert.equal(headerSource.includes('textContent = "EditorLab V2"'), false);
    assert.equal(headerSource.includes("showEditorLabV2"), false);
    assert.equal(headerSource.includes('data-ui-v2-id", "editorlabv2.button"'), false);
    assert.equal(
