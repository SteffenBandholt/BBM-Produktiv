...
async function runSigekoModuleBoundaryTests(run) {
  ...
  await run("S1.1: produktiver IPC-Registrar erreicht SiGeKo-Service ohne Protokoll", () => {
    ...
    assert.deepEqual([...harness.handlers.keys()], ["sigeko:getPreNotificationRecipients", "sigeko:savePreNotificationRecipients", "sigeko:getPreNotificationCompletion", "sigeko:savePreNotificationCompletion", "sigeko:openSimplePreNotificationMail", "sigeko:getPreNotificationWorkflow", "sigeko:preparePreNotificationMail", "sigeko:importPreNotificationSignedReturn", "sigeko:openPreNotificationSignedReturn", "sigeko:openPreNotificationMailDraft", "sigeko:previewPreNotificationPdf", "sigeko:createPreNotificationPdf", "sigeko:listPreNotificationDocuments", "sigeko:openPreNotificationDocumentFile", "sigeko:preparePreNotificationPdfEditor", "sigeko:getStoragePaths", "sigeko:ensureStorageDirectories", "sigeko:openStorageDirectory", "sigeko:getCoordinatorProfile", "sigeko:saveCoordinatorProfile", "sigeko:getProjectData", "sigeko:saveProjectData", "sigeko:getReadiness", "sigeko:listAuthorityRecords", "sigeko:getAuthorityRecord", "sigeko:saveAuthorityRecord", "sigeko:confirmAuthorityRecord", "sigeko:markAuthorityUncertain", "sigeko:getProjectAuthorities", "sigeko:assignProjectAuthority", "sigeko:applyKnownProjectAuthorities", "sigeko:getPreNotification", "sigeko:savePreNotification", "sigeko:getModuleInfo"]);
  ...
  await run("S1.1: echter Preload-Aufruf durchlaeuft modularen Guard und Service", async () => {
    ...
    assert.deepEqual(Object.keys(exposed.bbmDb).filter((key) => key.startsWith("sigeko")), ["sigekoGetStoragePaths", "sigekoEnsureStorageDirectories", "sigekoOpenStorageDirectory", "sigekoGetModuleInfo", "sigekoGetCoordinatorProfile", "sigekoSaveCoordinatorProfile", "sigekoGetProjectData", "sigekoGetReadiness", "sigekoGetPreNotification", "sigekoSavePreNotification", "sigekoPreviewPreNotificationPdf", "sigekoCreatePreNotificationPdf", "sigekoListPreNotificationDocuments", "sigekoOpenPreNotificationDocumentFile", "sigekoPreparePreNotificationPdfEditor", "sigekoGetPreNotificationRecipients", "sigekoSavePreNotificationRecipients", "sigekoGetPreNotificationCompletion", "sigekoSavePreNotificationCompletion", "sigekoOpenSimplePreNotificationMail", "sigekoGetPreNotificationWorkflow", "sigekoPreparePreNotificationMail", "sigekoImportPreNotificationSignedReturn", "sigekoOpenPreNotificationSignedReturn", "sigekoOpenPreNotificationMailDraft", "sigekoSaveProjectData", "sigekoListAuthorityRecords", "sigekoGetAuthorityRecord", "sigekoSaveAuthorityRecord", "sigekoConfirmAuthorityRecord", "sigekoMarkAuthorityUncertain", "sigekoGetProjectAuthorities", "sigekoAssignProjectAuthority", "sigekoApplyKnownProjectAuthorities"]);
    ...
    for (const api of ["sigekoPreviewPreNotificationPdf", "sigekoCreatePreNotificationPdf", "sigekoListPreNotificationDocuments",
      "sigekoOpenPreNotificationDocumentFile", "sigekoPreparePreNotificationPdfEditor", "sigekoGetPreNotificationRecipients", "sigekoSavePreNotificationRecipients", "sigekoGetPreNotificationCompletion", "sigekoSavePreNotificationCompletion", "sigekoOpenSimplePreNotificationMail", "sigekoGetPreNotificationWorkflow",
      "sigekoPreparePreNotificationMail", "sigekoImportPreNotificationSignedReturn", "sigekoOpenPreNotificationSignedReturn", "sigekoOpenPreNotificationMailDraft"]) {
      await assert.rejects(exposed.bbmDb[api]({ projectId: "p", expectedRevision: 0 }), { code: "MODULE_NOT_ACTIVE" });
    }
  });
  ...
  await run("SiGeKo: Migrationsregistrar erhält Bestand und ergänzt nur eigene Fachtabellen", () => {
    ...
    try {
      ...
      assert.deepEqual(added.filter(row => row.type === "table").map(row => row.name), ["sigeko_authority_records", "sigeko_documents", "sigeko_pre_notification_workflows", "sigeko_pre_notifications", "sigeko_profiles", "sigeko_project_authorities", "sigeko_projects"]);
  ...
  await run("S1.1: technische Grenze enthaelt weder Fachdaten noch fremde Fachimporte", () => {
    ...
    assert.match(read("src/main/modules/sigeko/registerMigrations.js"), /require\("\.\.\/\.\.\/db\/sigekoSchema"\)/);
    assert.doesNotMatch(read("src/main/modules/sigeko/registerMigrations.js"), /protokoll|rechnung|restarbeiten|new Database/);
