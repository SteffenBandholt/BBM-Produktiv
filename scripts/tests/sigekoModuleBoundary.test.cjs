...
async function runSigekoModuleBoundaryTests(run) {
  ...
  await run("SiGeKo: Migrationsregistrar erhält Bestand und ergänzt nur eigene Fachtabellen", () => {
    ...
    try {
      ...
      assert.deepEqual(added.filter(row => row.type === "table").map(row => row.name), ["sigeko_authority_records", "sigeko_profiles", "sigeko_project_authorities", "sigeko_projects"]);
