...
async function runM830ComponentContractTests(run) {
  ...
  await run("M83.0 BBM 05: Restarbeiten-Liste, Editbox und Protokoll sind vollstaendig gebuendelt", () => {
    assert.deepEqual(Object.fromEntries(contracts.map((component) => [component.componentId, component.slots.length])), { "bbm.restarbeiten.filterbar": 31, "bbm.restarbeiten.list": 32, "bbm.restarbeiten.editbox": 53, "bbm.protokoll.screen": 10, "bbm.protokoll.quicklane": 19, "bbm.protokoll.list.shell": 6, "bbm.protokoll.list.columns": 26, "bbm.protokoll.editbox": 36 });
  });
