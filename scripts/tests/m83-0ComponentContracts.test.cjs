...
async function runM830ComponentContractTests(run) {
  ...
  try {
    ...
    await run("M83.0 Rechnung 01: echter Rechnungsscreen mountet alle 81 Einzel-Refs mit vollstaendigem DOM-Vertrag", () => {
      ...
      assert.equal(component.slots.length, 81);
      assert.equal(new Set(renderedIds).size, 81);
