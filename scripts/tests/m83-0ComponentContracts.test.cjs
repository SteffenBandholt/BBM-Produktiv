...
async function runM830ComponentContractTests(run) {
  ...
  try {
    ...
    await run("M83.0 Rechnung 01: echter Rechnungsscreen mountet alle 61 Einzel-Refs mit vollstaendigem DOM-Vertrag", () => {
      ...
      assert.equal(expectedIds.length, 61);
      assert.equal(component.slots.length, 61);
      assert.equal(new Set(renderedIds).size, 61);
