...
async function runRechnungNavigationTests(run) {
  ...
  await run("Rechnung Navigation: DRAFT-Eingaben speichern seriell ohne Positionsverlust", async () => {
    ...
    try {
      ...
      const first = { id: "position-1", type: "service", is_title: false, parent_id: null, short_text: "Alt", long_text: "", quantity: "1", unit: "St", unit_price_cents: 1000, is_nep: false, catalog_item_id: "catalog-1", source_offer_id: "offer-1", source_offer_position_id: "offer-position-1", source_order_id: "order-1", source_order_position_id: "order-position-1" };
