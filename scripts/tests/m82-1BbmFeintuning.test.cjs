...
async function runM821BbmFeintuningTests(run) {
  ...
  await run("M82.1 BBM 01: Registryversion bleibt konsistent", () => assert.equal(registry.BBM_M80_REGISTRY_VERSION, 26));
  ...
  await run("M82.1/M82.6 BBM 04: Restarbeiten, Protokoll und Rechnung besitzen sieben aktive Scopes", () => assert.deepEqual(registry.BBM_M80_ACTIVE_SCOPES, ["restarbeiten.header.root", "restarbeiten.list.root", "restarbeiten.edit.root", "protokoll.screen.root", "protokoll.list.root", "protokoll.edit.root", "rechnung.screen", "sigeko.screen", "sigeko.preNotification", "projektverwaltung.plannedStart", "projektverwaltung.builder"]));
