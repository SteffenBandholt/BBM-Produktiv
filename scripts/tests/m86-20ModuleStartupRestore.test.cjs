...
async function runM8620ModuleStartupRestoreTests(run) {
  let report;
  await run("M86.20: beide Module wenden ihr gespeichertes Layout beim normalen Modulstart genau einmal an", () => {
    ...
    assert.equal(report.loads.length, 2, "Jedes Modulprofil wird genau einmal geladen.");
  });
