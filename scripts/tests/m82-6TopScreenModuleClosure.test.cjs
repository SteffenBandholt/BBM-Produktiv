...
async function runM826TopScreenModuleClosureTests(run) {
  ...
  await run("M82.6 BBM 24: logische Listenspalten verwenden echte Tabellenbindungen und wiederkehrende Teile vorhandene Multi-Refs", () => { assert.match(protocolList, /registerM80TableColumnRef\(/); assert.match(protocolList, /this\._uiEditorRefs\[column\.key\] \|\| \[\]/); assert.match(protocolList, /registerM80MultiRef\(id, this\._uiEditorRefs\[id\] \|\| \[\], this\.root/); assert.match(protocolList, /column\.widthVariable/); assert.doesNotMatch(protocolList.slice(protocolList.indexOf("_registerUiEditorColumnRefs()"), protocolList.indexOf("_renderRow(")), /createElement|appendChild|insertBefore|querySelector/); });
  ...
  await run("M82.6 BBM 26: vorhandener Protokoll-Tabellenvertrag bleibt Ausgangsbreite und der UI-Editor besitzt getrennte Breitenquellen", () => { assert.match(protocolLayout, /tableKey:\s*"protokoll_tops"/); assert.match(protocolLayout, /PROTOKOLL_TOPS_COLUMNS[\s\S]*?key:\s*"topNumber"[\s\S]*?key:\s*"shortText"[\s\S]*?key:\s*"meta"/); assert.match(protocolList, /--bbm-ui-editor-tops-list-number-col/); assert.match(protocolList, /--bbm-ui-editor-tops-list-text-col/); assert.match(protocolList, /--bbm-ui-editor-tops-list-meta-col/); });
