...
const UI_EDITOR_HINT_INFOTEXT_HOST_PREVIEW_REFERENCE_DOC_PATH = path.join(
  __dirname,
  "../../docs/UI_EDITOR_HINWEIS_INFOTEXT_HOST_PREVIEW_REFERENZSTAND.md"
);
const UI_EDITOR_HINT_INFOTEXT_ELEMENTMODEL_PREVIEW_REFERENCE_DOC_PATH = path.join(
  __dirname,
...
const UI_EDITOR_HINT_INFOTEXT_DRAFT_RESET_REFERENCE_DOC_PATH = path.join(
  __dirname,
...
const UI_EDITOR_HINT_INFOTEXT_PAYLOAD_PREVIEW_REFERENCE_DOC_PATH = path.join(
  __dirname,
...
const UI_EDITOR_HINT_INFOTEXT_SAVE_EXECUTION_REFERENCE_DOC_PATH = path.join(
  __dirname,
...
const UI_EDITOR_HINT_INFOTEXT_SAVE_BUTTON_GATED_TEST_RELEASE_REFERENCE_DOC_PATH = path.join(
  __dirname,
...
const UI_EDITOR_HINT_INFOTEXT_PRODUCTIVE_SAVE_ACTIVATION_REFERENCE_DOC_PATH = path.join(
  __dirname,
...
function matchesSelector(node, selector) {
  ...
  if (raw === "[data-ui-editor-hint-infotext-host-preview=\"true\"]") {
    return node.getAttribute("data-ui-editor-hint-infotext-host-preview") === "true";
  }
  ...
  if (raw === "[data-ui-editor-hint-infotext-save-execution-preview=\"true\"]") {
    return node.getAttribute("data-ui-editor-hint-infotext-save-execution-preview") === "true";
  }
  ...
  if (raw === "[data-ui-editor-hint-infotext-draft-reset=\"true\"]") {
    return node.getAttribute("data-ui-editor-hint-infotext-draft-reset") === "true";
  }
...
async function runBbmUiEditorRuntimeLauncherTests(run) {
  ...
  await run("BBM UI-Editor-Runtime: zeigt kompakte read-only Surface-Auswahl, Hinweis und unveraenderte SurfaceInfo", async () => {
    ...
    const hintInfotextSaveExecutionPreview = doc.querySelector(
      '[data-ui-editor-hint-infotext-save-execution-preview="true"]'
    );
    const hintInfotextSaveButtonStatePreview = doc.querySelector(
    ...
    assert.equal(hintInfotextDraftInput.disabled, false);
    assert.equal(hintInfotextDraftInput.readOnly === true, false);
    assert.notEqual(hintInfotextDraftInput.getAttribute("readonly"), "true");
    assert.notEqual(hintInfotextDraftInput.getAttribute("disabled"), "true");
    assert.notEqual(hintInfotextDraftInput.style.pointerEvents, "none");
    const previewPanel = getPreviewPanel(doc);
    const draftInputMouseDown = {
      type: "mousedown",
      target: hintInfotextDraftInput,
      preventDefault() {
        this.defaultPrevented = true;
      },
      stopPropagation() {
    ...
    previewPanel.dispatchEvent(draftInputMouseDown);
    assert.equal(draftInputMouseDown.defaultPrevented === true, false);
    assert.equal(draftInputMouseDown.stopped, true);
    ...
    assert.equal(Boolean(hintInfotextHostPreview), true);
    assert.equal(
      hintInfotextHostPreview.textContent,
      "Hinweis / Infotext\nDies ist ein nicht gespeicherter Hinweis-Entwurf.\nnicht gespeichert"
    );
    assert.equal(Boolean(hintInfotextElementModelPreview), true);
    assert.equal(
      hintInfotextElementModelPreview.textContent,
      "Typ: Hinweis / Infotext\nSurface: restarbeiten.ui.main\nStatus: nicht gespeichert\nDies ist ein nicht gespeicherter Hinweis-Entwurf."
    );
    assert.equal(Boolean(hintInfotextPayloadPreview), true);
    assert.equal(
    ...
    assert.equal(hintInfotextDraftPreview.querySelectorAll("button").length, 1);
    ...
    const updatedHintInfotextPayloadPreview = doc.querySelector('[data-ui-editor-hint-infotext-payload-preview="true"]');
    ...
    assert.equal(
      updatedHintInfotextHostPreview.textContent,
    ...
    hintInfotextDraftReset.click();
    const resetHintInfotextDraftInput = doc.querySelector('[data-ui-editor-hint-infotext-draft-input="true"]');
    const resetHintInfotextLivePreview = doc.querySelector('[data-ui-editor-hint-infotext-live-preview="true"]');
    const resetHintInfotextHostPreview = doc.querySelector('[data-ui-editor-hint-infotext-host-preview="true"]');
    ...
    assert.equal(
      resetHintInfotextHostPreview.textContent,
      "Hinweis / Infotext\nDies ist ein nicht gespeicherter Hinweis-Entwurf.\nnicht gespeichert"
    );
    assert.equal(
      resetHintInfotextElementModelPreview.textContent,
      "Typ: Hinweis / Infotext\nSurface: restarbeiten.ui.main\nStatus: nicht gespeichert\nDies ist ein nicht gespeicherter Hinweis-Entwurf."
    );
    assert.equal(
      resetHintInfotextPayloadPreview.textContent,
      "type: Hinweis / Infotext\nsurfaceId: restarbeiten.ui.main\nstatus: draft\npersisted: false\nDies ist ein nicht gespeicherter Hinweis-Entwurf."
    );
    assert.equal(
  ...
  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Host-Vorschau bleibt vorhanden", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_HOST_PREVIEW_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Host-Vorschau-Referenz fehlt."
    );
    const docSource = fs.readFileSync(UI_EDITOR_HINT_INFOTEXT_HOST_PREVIEW_REFERENCE_DOC_PATH, "utf8");
    ...
    for (const required of [
      ...
      assert.equal(
        docSource.includes(required),
        true,
        `Hinweis-/Infotext-Host-Vorschau enthaelt ${required} nicht.`
      );
    }
  });

  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Elementmodell-Vorschau bleibt vorhanden", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_ELEMENTMODEL_PREVIEW_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Elementmodell-Vorschau-Referenz fehlt."
    );
    const docSource = fs.readFileSync(UI_EDITOR_HINT_INFOTEXT_ELEMENTMODEL_PREVIEW_REFERENCE_DOC_PATH, "utf8");
  ...
  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Payload-Vorschau bleibt vorhanden", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_PAYLOAD_PREVIEW_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Payload-Vorschau-Referenz fehlt."
    );
    const docSource = fs.readFileSync(UI_EDITOR_HINT_INFOTEXT_PAYLOAD_PREVIEW_REFERENCE_DOC_PATH, "utf8");
  ...
  await run("BBM UI-Editor-Runtime: Speicherbereitschaft zeigt Host-Kontext, bleibt aber gesperrt", async () => {
    ...
    const saveExecutionPreview = doc.querySelector('[data-ui-editor-hint-infotext-save-execution-preview="true"]');
    ...
    getPreviewPanel(doc).dispatchEvent(validHostDraftMouseDown);
    assert.equal(validHostDraftMouseDown.defaultPrevented === true, false);
    assert.equal(validHostDraftMouseDown.stopped, true);
    const sichtpruefungText = "G137 Sichtpruefung Hinweis Infotext";
    draftInput.value = sichtpruefungText;
    ...
    assert.equal(source.includes("writeReleaseEnabled: true"), true);
    assert.equal(source.includes("gateOpen: writeGate.gateOpen === true"), true);
    assert.equal(source.includes("buttonEnabled: buttonState.buttonEnabled === true"), true);
    assert.equal(source.includes("process.env"), false);
    assert.equal(source.includes("saveAdapter.targetMethod("), false);
    assert.equal(source.includes("targetMethod("), false);
  ...
  await run("BBM UI-Editor-Runtime: Speicherbutton-Aktivierung bleibt Standard-deaktiviert und nur testseitig positiv", async () => {
    const mod = await loadRuntime();
    const noteText = "Dies ist ein kontrollierter Button-Aktivierungstest.";
    const validHostContext = mod.normalizeHostContextStatus({
      projectId: "project-42",
      restarbeitId: "restarbeit-99",
      targetContext: "Restarbeiten",
    ...
    assert.equal(noHostState.buttonEnabled, false);
    assert.equal(noHostState.reason, "Host-Kontext fehlt");
    assert.equal(noHostState.blockReasons.includes("Payload unvollständig"), true);
    assert.equal(noHostState.blockReasons.includes("restarbeitId fehlt"), true);

    const missingRestarbeitState = mod.buildReadonlyHintInfotextSaveButtonState({
      value: noteText,
      hostContextStatus: mod.normalizeHostContextStatus({
        projectId: "project-42",
        restarbeitId: "",
        targetContext: "Restarbeiten",
        targetSurfaceId: "restarbeiten.ui.main",
    ...
    assert.equal(missingRestarbeitState.buttonEnabled, false);
    assert.equal(missingRestarbeitState.blockReasons.includes("restarbeitId fehlt"), true);

    const emptyTextState = mod.buildReadonlyHintInfotextSaveButtonState({
      value: "   ",
      hostContextStatus: validHostContext,
      testOnly: {
    ...
    assert.equal(emptyTextState.buttonEnabled, false);
    assert.equal(emptyTextState.payloadComplete, false);
    assert.equal(emptyTextState.hintTextValid, false);
    assert.equal(emptyTextState.blockReasons.includes("Hinweistext fehlt"), true);

    const positiveState = mod.buildReadonlyHintInfotextSaveButtonState({
      value: noteText,
      hostContextStatus: validHostContext,
      testOnly: {
        mode: "save-button-gated-test-release",
    ...
    assert.deepEqual(positiveState, {
      buttonPrepared: true,
      buttonEnabled: true,
      reason: "alle Speicherbedingungen erfüllt",
      blockReasons: [],
      standardPath: "gated",
    ...
    for (const [label, testOnly] of [
      ["DEV-Kontext", { mode: "dev", writeReleaseEnabled: true, gateOpen: true, adapterAvailable: true }],
      ["ENV-artiger Kontext", { mode: "env", writeReleaseEnabled: true, gateOpen: true, adapterAvailable: true }],
      ["nur vorhandene Payload", null],
      ["vorhandener Adapter ohne Freigabe", { adapterAvailable: true }],
      ["vorhandene restarbeitId ohne Gate", { mode: "save-button-gated-test-release", writeReleaseEnabled: true }],
      [
      ...
      const state = mod.buildReadonlyHintInfotextSaveButtonState({
        value: noteText,
        hostContextStatus: validHostContext,
        testOnly,
      });
      assert.equal(state.buttonEnabled, false, `${label} darf den Button nicht aktivieren.`);
      assert.equal(state.standardPath, "gated", `${label} bleibt hinter dem Gate.`);
    ...
    let writeCalled = false;
    const saveButtonState = mod.buildReadonlyHintInfotextSaveButtonState({
      value: noteText,
      hostContextStatus: validHostContext,
      testOnly: {
        mode: "save-button-gated-test-release",
    ...
    assert.equal(saveButtonState.buttonEnabled, true);
    assert.equal(writeCalled, false);

    const source = fs.readFileSync(RUNTIME_PATH, "utf8");
    assert.equal(source.includes("buildReadonlyHintInfotextSaveButtonState"), true);
    assert.equal(source.includes("save-button-gated-test-release"), true);
    assert.equal(source.includes("button.disabled = initialButtonState.buttonEnabled !== true"), true);
    assert.equal(source.includes("buttonEnabled: buttonState.buttonEnabled === true"), true);
    assert.equal(source.includes("process.env"), false);
    assert.equal(source.includes("window.bbmDb.restarbeitenCreateNote("), false);
    assert.equal(source.includes(".restarbeitenCreateNote("), false);
  ...
  await run("BBM UI-Editor-Runtime: Speicherklick-Pfad bleibt Standard-blockiert und nur testseitig positiv", async () => {
    ...
    assert.equal(standardClickState.gateOpen, true);
    ...
    const productiveRuntimeCalls = [];
    const productiveRuntime = {
      win: {
        bbmDb: {
          async restarbeitenCreateNote(payload) {
            productiveRuntimeCalls.push(payload);
            return { ok: true, note: { id: "note-runtime-1" } };
          },
        },
      },
      clickState: {},
    };
    const productiveButtonState = mod.buildReadonlyHintInfotextSaveButtonState({
      value: noteText,
      hostContextStatus: validHostContext,
    ...
    const productiveRuntimeResult = await mod.executeReadonlyHintInfotextSaveClick({
      value: noteText,
      hostContextStatus: validHostContext,
  ...
  await run("BBM UI-Editor-Runtime: Speicherbereitschaft aktualisiert Host-Kontext beim Oeffnen", async () => {
    ...
    assert.equal(storageCheck.textContent.includes("Schreibweg freigegeben: ja"), true);
    assert.equal(storageCheck.textContent.includes("Speichern: bereit"), true);
  ...
  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Schreibfreigabe-Gate bleibt dokumentiert", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_WRITE_GATE_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Schreibfreigabe-Gate-Referenz fehlt."
    );
    const docSource = fs.readFileSync(UI_EDITOR_HINT_INFOTEXT_WRITE_GATE_REFERENCE_DOC_PATH, "utf8");
  ...
  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Save-Ausfuehrung bleibt dokumentiert", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_SAVE_EXECUTION_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Save-Ausfuehrung-Referenz fehlt."
    );
    const docSource = fs.readFileSync(UI_EDITOR_HINT_INFOTEXT_SAVE_EXECUTION_REFERENCE_DOC_PATH, "utf8");
    ...
    for (const required of [
      ...
      "keine DEV-Modus-Aktivierung",
      "UI-Editor-kit bleibt unveraendert",
      "bewusst kein isolierter Fake-Adapter-Positivtest",
    ]) {
      assert.equal(
        docSource.includes(required),
        true,
  ...
  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Speicherbutton-Gated-Testfreigabe bleibt dokumentiert", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_SAVE_BUTTON_GATED_TEST_RELEASE_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Speicherbutton-Gated-Testfreigabe-Referenz fehlt."
    );
    const docSource = fs.readFileSync(
      UI_EDITOR_HINT_INFOTEXT_SAVE_BUTTON_GATED_TEST_RELEASE_REFERENCE_DOC_PATH,
      "utf8"
    );
  ...
  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Produktiv-Save-Aktivierung bleibt dokumentiert", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_PRODUCTIVE_SAVE_ACTIVATION_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Produktiv-Save-Aktivierungsreferenz fehlt."
    );
    const docSource = fs.readFileSync(
      UI_EDITOR_HINT_INFOTEXT_PRODUCTIVE_SAVE_ACTIVATION_REFERENCE_DOC_PATH,
      "utf8"
    );
  ...
  await run("BBM UI-Editor-Runtime: Hinweis-/Infotext-Reset bleibt vorhanden", async () => {
    assert.equal(
      fs.existsSync(UI_EDITOR_HINT_INFOTEXT_DRAFT_RESET_REFERENCE_DOC_PATH),
      true,
      "Hinweis-/Infotext-Reset-Referenz fehlt."
    );
    const docSource = fs.readFileSync(UI_EDITOR_HINT_INFOTEXT_DRAFT_RESET_REFERENCE_DOC_PATH, "utf8");
