...
const { FirmDirectoryService } = require(path.join(rootPath, "src/main/domain/firms/FirmDirectoryService.js"));
...
function fixture() {
  ...
  db.exec(`
    ...
    CREATE TABLE user_profile (
      ...
      country TEXT, phone TEXT, email TEXT, tax_number TEXT, vat_id TEXT, iban TEXT, bic TEXT, bank_name TEXT
    );
    INSERT INTO projects (id, name) VALUES ('p1', 'Projekt Eins');
    INSERT INTO user_profile (id, name1, street, zip, city) VALUES (1, 'BBM', 'Werkweg 1', '10115', 'Berlin');
  `);
  ...
  return { db, service, directory, close() { db.close(); fs.rmSync(root, { recursive: true, force: true }); } };
}

function insertFirm(db, id, name, fields = {}) {
  db.prepare(`
    INSERT INTO firms (id, name, street, zip, city, email, use_project_participant, use_customer, created_at, updated_at)
    VALUES (@id, @name, @street, @zip, @city, @email, @participant, @customer, 'now', 'now')
  `).run({
    id,
    name,
...
function insertLegacyDraft(db, { id, firmId, projectId = "p1", status = "DRAFT", snapshot = null }) {
  db.prepare(`
    INSERT INTO invoices (
      ...
      service_period_type, service_date, customer_ref_kind, customer_firm_id, customer_project_id,
      positions_json, payment_term_days, due_date, customer_snapshot_json, issuer_snapshot_json,
  ...
  `).run({
    ...
    firmId,
    projectId,
    snapshot: status === "BOOKED" ? JSON.stringify(snapshot || { companyName: "Historischer Kunde" }) : null,
    issuer: status === "BOOKED" ? JSON.stringify({ companyName: "BBM" }) : null,
  });
}

async function runRechnungCentralCustomersTests(run) {
  await run("Rechnung R2-I1 01-07: Kundenrolle ist zentral, projektunabhängig und duplikatfrei", async () => {
    const env = fixture();
    try {
      insertFirm(env.db, "customer", "Nur Kunde", { customer: true });
      insertFirm(env.db, "both", "Kunde und Teilnehmer", { customer: true, participant: true });
      insertFirm(env.db, "participant", "Nur Teilnehmer", { participant: true });
      insertFirm(env.db, "none", "Ohne Rolle");
      env.db.prepare("INSERT INTO project_global_firms (project_id, firm_id) VALUES ('p1', 'customer'), ('p1', 'both')").run();
      ...
      const projectDraft = await env.service.createDraft(invoiceInput("both", { project_id: "p1" }));
      ...
      await assert.rejects(
        () => env.service.createDraft(invoiceInput("legacy", {
  ...
  await run("Rechnung R2-I1 08-10: Buchung friert den zentralen Kundensnapshot trotz späterer Änderungen ein", async () => {
    const env = fixture();
    try {
      insertFirm(env.db, "customer", "Kunde Vorher", {
        customer: true, street: "Altweg 1", zip: "12345", city: "Altstadt", email: "alt@example.test",
      });
      ...
      assert.deepEqual(booked.customer_snapshot.source, { kind: "global_firm", id: "customer", projectId: null });
      assert.equal(booked.customer_snapshot.companyName, "Kunde Vorher");
      env.db.prepare("UPDATE firms SET name = 'Kunde Nachher', street = 'Neuweg 9' WHERE id = 'customer'").run();
      usagesRepo.setUsage({ firmId: "customer", usageCode: usagesRepo.FIRM_USAGE_CODES.INVOICE_CUSTOMER, enabled: false, dbConn: env.db });
  ...
  await run("Rechnung R2-I1 11-12: Entwurfsmigration ordnet nur eindeutige Altverweise zu und lässt Buchungen unangetastet", () => {
    const env = fixture();
    try {
      ...
      assert.deepEqual(
        env.db.prepare("SELECT customer_ref_kind, customer_firm_id, customer_project_id FROM invoices WHERE id = 'draft-clear'").get(),
        { customer_ref_kind: "global_firm", customer_firm_id: "central-clear", customer_project_id: null }
      );
      assert.deepEqual(
        env.db.prepare("SELECT customer_ref_kind, customer_firm_id, customer_project_id FROM invoices WHERE id = 'draft-ambiguous'").get(),
        { customer_ref_kind: "project_firm", customer_firm_id: "local-ambiguous", customer_project_id: "p1" }
      );
      assert.deepEqual(
        env.db.prepare("SELECT status, customer_ref_kind, customer_firm_id FROM invoices WHERE id = 'booked-legacy'").get(),
        { status: "BOOKED", customer_ref_kind: "project_firm", customer_firm_id: "local-booked" }
      );
      assert.equal(result.customerMigration.projectRefsMigrated, 1);
      assert.equal(result.customerMigration.unresolvedProjectRefs, 2);
      assert.equal(usagesRepo.hasUsage({ firmId: "central-clear", usageCode: usagesRepo.FIRM_USAGE_CODES.INVOICE_CUSTOMER, dbConn: env.db }), true);
      ...
      assert.equal(env.db.prepare("SELECT COUNT(*) AS count FROM invoices").get().count, 4);
    } finally { env.close(); }
  });
}
