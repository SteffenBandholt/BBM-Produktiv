...
function _buildTable(page, data) {
  ...
  if (type === "invoice") {
    markInvoiceEditorElement(table, {
  ...
  if (type === "invoice") {
    markInvoiceEditorElement(tbody, {
...
export function renderPrint({ pages, data } = {}) {
  ...
  (pages || []).forEach((page, idx) => {
    ...
    if (normalizedMode === "invoice") {
      markInvoiceEditorElement(pageEl, {
    ...
    if (pageNo === 1) {
      ...
      pageEl.appendChild(renderV2MiniHeader({
        data: runtimeData,
        pageNo,
        totalPages,
        modeLabel,
        content: normalizedMode === "invoice" ? buildInvoiceMiniHeaderContent(runtimeData.invoice || {}) : null,
      }));
    }
    ...
    if (normalizedMode === "invoice") {
      ...
      if (page?.invoiceFirst) {
        const invoiceIntro = buildInvoiceBodyIntro(runtimeData.invoice || {});
