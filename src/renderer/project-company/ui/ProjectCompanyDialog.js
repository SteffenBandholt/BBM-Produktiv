import { ProjectCompanyForm } from "./ProjectCompanyForm.js";
import { mkEl } from "./dom.js";

export class ProjectCompanyDialog {
  constructor({ documentRef, title = "Projektfirma bearbeiten", onSubmit, onCancel } = {}) {
  ...
  _build(title) {
    ...
    this.root = mkEl(doc, "section", "project-company-dialog");
    this.root.setAttribute("data-component", "project-company-dialog");
    this.titleEl = mkEl(doc, "h3", "project-company-dialog-title", title);
    this.form = new ProjectCompanyForm({ documentRef: doc });

    this.actions = mkEl(doc, "div", "project-company-dialog-actions");
    this.saveBtn = mkEl(
      doc,
      "button",
      "project-company-dialog-btn project-company-dialog-btn-save",
      "Speichern"
    );
    this.cancelBtn = mkEl(
      doc,
      "button",
      "project-company-dialog-btn project-company-dialog-btn-cancel",
      "Abbrechen"
    );
