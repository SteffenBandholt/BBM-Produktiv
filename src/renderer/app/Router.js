...
export default class Router {
  ...
  async showTops(meetingId, projectId, options = {}) {
    ...
    if (!meetingResult?.ok) { alert(meetingResult?.error || "Besprechung konnte nicht geladen werden."); return false; }
    const meeting = meetingResult.meeting;
    if (!meeting || meeting.project_id !== effectiveProjectId) { alert("Besprechung gehört nicht zum gewählten Projekt."); return false; }
    this.currentSeriesKey = normalizeSeriesKey(meeting.series_key);
    const projectsResult = await window.bbmDb.projectsList();
    if (!projectsResult?.ok) { alert(projectsResult?.error || "Projekt konnte nicht geladen werden."); return false; }
    const project = projectsResult.list.find(row => row.id === effectiveProjectId);
  ...
  async openProjectProtocol(projectId, options = {}) {
    ...
    if (projectResult && !projectResult.ok) return { ok: false, reason: "project-load-failed", error: projectResult.error };
    const project = projectResult?.list?.find(row => row.id === effectiveProjectId) || options.project;
    ...
    if (!meetingsRes?.ok) return { ok: false, reason: "meetings-load-failed", error: meetingsRes?.error };
  ...
  async showBbmUiEditorDemo() {
    if (globalThis.window?.bbmDistribution?.uiEditorEnabled === false) return { ok: false, reason: "UI_EDITOR_DISABLED_IN_DISTRIBUTION" };
    const ui = {
      node(tag) {
