...
const buttonReflowIdsByParent = Object.freeze({
  "rechnung.overview.header": Object.freeze(["rechnung.overview.title", "rechnung.overview.subtitle", "rechnung.overview.masterData", "rechnung.overview.new"]),
  "rechnung.masterData.header": Object.freeze(["rechnung.masterData.title", "rechnung.masterData.close"]),
  "rechnung.masterData.issuer": Object.freeze(["rechnung.masterData.issuer.save"]),
  "rechnung.masterData.customers": Object.freeze(["rechnung.masterData.customers.create", "rechnung.masterData.customers.edit"]),
  "rechnung.masterData.catalog": Object.freeze(["rechnung.masterData.catalog.create", "rechnung.masterData.catalog.save"]),
...
const baseElements = Object.freeze([
  ...
  action("rechnung.overview.masterData", "Stammdaten", "rechnung.overview.header", 25, "openMasterData"),
  ...
  area("rechnung.masterData", "Rechnungsstammdaten", "rechnung.screen.content", 26, "masterData"),
  group("rechnung.masterData.header", "Kopf Rechnungsstammdaten", "rechnung.masterData", 27, "header"),
  label("rechnung.masterData.title", "Stammdaten", "rechnung.masterData.header", 28),
  action("rechnung.masterData.close", "Stammdaten schließen", "rechnung.masterData.header", 29, "closeMasterData"),
  group("rechnung.masterData.issuer", "Rechnungstellerprofil", "rechnung.masterData", 30, "formGroup"),
  label("rechnung.masterData.issuer.summary", "Rechnungstellerprofil", "rechnung.masterData.issuer", 31),
  ...[["legalName", "Firmen-/Rechtsname"], ["additionalName", "Namenszusatz"], ["street", "Straße"], ["zip", "PLZ"], ["city", "Ort"], ["country", "Land"], ["phone", "Telefon"], ["email", "E-Mail"], ["website", "Website"], ["taxNumber", "Steuernummer"], ["vatId", "USt-IdNr."], ["iban", "IBAN"], ["bic", "BIC"], ["bankName", "Bank"], ["commercialRegister", "Registergericht"], ["registerNumber", "Registernummer"], ["managingDirector", "Geschäftsführung"], ["legalNotice", "Rechtlicher Hinweis"]].map(([key, name], index) => field(`rechnung.masterData.issuer.${key}`, name, "rechnung.masterData.issuer", 32 + index, key === "legalNotice" ? "multilineText" : "singleLineText")),
  action("rechnung.masterData.issuer.save", "Rechnungsteller speichern", "rechnung.masterData.issuer", 51, "saveIssuerProfile"),
  group("rechnung.masterData.customers", "Rechnungskunden", "rechnung.masterData", 60, "formGroup"),
  field("rechnung.masterData.customers.select", "Rechnungskunde auswählen", "rechnung.masterData.customers", 61, "select"),
  action("rechnung.masterData.customers.create", "Kunde anlegen", "rechnung.masterData.customers", 62, "createCustomer"),
  action("rechnung.masterData.customers.edit", "Kunde bearbeiten", "rechnung.masterData.customers", 63, "editCustomer"),
  group("rechnung.masterData.catalog", "Leistungskatalog", "rechnung.masterData", 70, "formGroup"),
  field("rechnung.masterData.catalog.select", "Katalogleistung auswählen", "rechnung.masterData.catalog", 71, "select"),
  field("rechnung.masterData.catalog.shortText", "Kurztext", "rechnung.masterData.catalog", 72, "singleLineText"),
  field("rechnung.masterData.catalog.longText", "Langtext", "rechnung.masterData.catalog", 73, "multilineText"),
  field("rechnung.masterData.catalog.unit", "Einheit", "rechnung.masterData.catalog", 74, "singleLineText"),
  field("rechnung.masterData.catalog.unitPrice", "Einzelpreis", "rechnung.masterData.catalog", 75, "currency"),
  field("rechnung.masterData.catalog.vatRate", "Mehrwertsteuer", "rechnung.masterData.catalog", 76, "readOnlyText"),
  action("rechnung.masterData.catalog.create", "Neue Katalogleistung", "rechnung.masterData.catalog", 77, "createCatalogEntry"),
  action("rechnung.masterData.catalog.save", "Katalogleistung speichern", "rechnung.masterData.catalog", 78, "saveCatalogEntry"),
