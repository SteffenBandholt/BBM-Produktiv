...
export default class ProjectFormScreen {
  ...
  async _archive() {
    ...
    try {
      ...
      // nach Archivieren direkt zurück zu ProjectsScreen
  ...
  async _notifySaved() {
    ...
    try {
      ...
      console.warn("[ProjectFormScreen] onSaved handler failed", err);
    }
  }
  ...
  async load() {
    ...
    try {
      ...
      try {
        ...
        if (!response?.ok || !Array.isArray(response.list)) throw new Error(response?.error || "Protokolle konnten nicht geladen werden.");
