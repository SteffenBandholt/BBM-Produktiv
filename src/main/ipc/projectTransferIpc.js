...
function _validateMeetingSeriesPayload(payload, manifest) {
  ...
  for (const row of tops.values()) {
    ...
    if (row.parent_top_id !== null && row.parent_top_id !== undefined) {
      ...
      while (current) {
        ...
        current = current.parent_top_id === null || current.parent_top_id === undefined ? null : tops.get(current.parent_top_id);
      }
    }
  }
  ...
  for (const row of meetingTops) {
    ...
    if (row.completed_in_meeting_id !== null && row.completed_in_meeting_id !== undefined) {
