...
class FirmDirectoryService {
  ...
  listProjectParticipants({ projectId, includeInactive = false } = {}) {
    ...
    const db = this._db();
    this.usageRepo.ensureFirmUsagesSchema(db);
    const locals = db
      .prepare(
    ...
    const globals = db
      .prepare(
        `SELECT f.*, pgf.is_active AS is_active, pgf.project_id AS assigned_project_id
         ...
         JOIN firm_usages fu
           ON fu.firm_id = f.id AND fu.usage_code = 'project_participant'
  ...
  update({ ref: refInput, patch } = {}) {
    ...
    return ref.kind === FIRM_KINDS.GLOBAL ? this.get(ref) : toDirectoryEntry(row, ref.kind);
  }
  ...
  setUses({ ref: refInput, uses, expectedUpdatedAt } = {}) {
    ...
    const transaction = db.transaction(() => {
      ...
      const scopeSql = " AND project_id = @project_id";
  ...
  prepareLocalToGlobal({ ref: refInput } = {}) {
    ...
    return Object.freeze({
      ...
      source,
      matchingGlobalFirms: matches.map((row) =>
        toDirectoryEntry(
          row,
          FIRM_KINDS.GLOBAL,
          this.usageRepo.listCodesByFirm(row.id, db)
