...
class TopService {
  ...
  moveTop({ topId, targetParentId }) {
    ...
    if (targetParentId) {
      ...
      if ((targetParent.series_key || "construction") !== (top.series_key || "construction") || !this.meetingTopsRepo.getMeetingTop(openMeeting.id, targetParent.id)) {
        throw new Error("Ziel-Parent gehört zu einer anderen Besprechungsreihe.");
      }
  ...
  updateMeetingFields({ meetingId, topId, patch }) {
    ...
    if (patch.status === undefined && requestedCompletion !== null && requestedCompletion !== undefined) {
